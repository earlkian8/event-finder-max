...
const Header = () => {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 bg-indigo-700 text-white transition-all duration-300 ${
          ...
          <nav className="hidden md:flex space-x-1">
            {navItems.map(({ path, label, icon: Icon }) => (
              <NavLink
                ...
                to={path}
                className={({ isActive }) =>
                  `group relative px-4 py-2 rounded-lg font-medium transition-all duration-200 flex items-center gap-2 ${
                    isActive
